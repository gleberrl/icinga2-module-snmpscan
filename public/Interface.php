...
echo "</script>";
	if ($SNMP_MODEL_T != "") {
		...
		echo "<pre>";
		echo '<table><tr><th>Interface Name |</th><th>Interface Full Name |</th><th>Interface Description |</th><th>Interface Admin Status</th>';
                $i=1;   
                while ( $i <= $SNMP_INT_NUMBER ) {
	                echo '<tr><td>'.$snmp_ifname[$i].'</td><td>'.$snmp_ifdescr[$i].'</td><td>'.$snmp_ifalias[$i].'</td><td>'.$snmp_ifstatus[$i].'</td></tr>';
			$i++;
		}
		echo '</table>';
		echo "</pre>";
