...
if ($SNMP_V == "1" | $SNMP_V == "2c") {
	...
	if(!empty($_POST['process'])) {
		if ($SO_T == "windows") {
				...
				system('icingacli director service create "'.$check.'" --imports "Processo Windows" --vars.win_proc_service "'.$check.'" --host "'.$host.'"');
                		echo $i." - ".$check."\n";
				$i++;
	        	}
		}
	...
	if(!empty($_POST['process'])) {
        	foreach($_POST['process'] as $check) {
                	echo "Processo ".$i." - ".$check." cadastrado com sucesso no Host: ".$host.".\n";
	                $i++;
        	}
	}
	echo "</pre>";
